...
namespace SushiBarDatabaseImplement.Implements
{
    public class OrderStorage : IOrderStorage
    {
        ...
        public List<OrderViewModel> GetFilteredList(OrderBindingModel model)
        {
            ...
            using (var context = new SushiBarDatabase())
            {
                return context.Orders.Include(rec => rec.Sushi).Include(rec => rec.Client).Include(rec => rec.Cook)
                    .Where(rec => (!model.DateFrom.HasValue && !model.DateTo.HasValue && rec.DateCreate.Date == model.DateCreate.Date) 
                    || (model.DateFrom.HasValue && model.DateTo.HasValue && rec.DateCreate.Date >= model.DateFrom.Value.Date && rec.DateCreate.Date <=
                    model.DateTo.Value.Date) || (model.ClientId.HasValue && rec.ClientId == model.ClientId) || 
                    (model.FreeOrders.HasValue && model.FreeOrders.Value && rec.Status == OrderStatus.Принят) || (model.CookId.HasValue && rec.CookId 
                    == model.CookId && rec.Status == OrderStatus.Выполняется) ||
                (model.CookId.HasValue && rec.CookId == model.CookId && rec.Status == OrderStatus.НужныИнгредиенты)).Select(rec => new OrderViewModel
                {
                    Id = rec.Id,
                    SushiId = rec.SushiId,
                    ClientId = rec.ClientId,
